<?php
	...
	{	
		...
		function handleForm()
		{
			...
			add_filter( 'wpmem_admin_tabs',array($this,'wpmem_add_Websms_tab'),99,1);
			add_action( 'wpmem_admin_do_tab',array($this,'wpmem_Websms_panel'),999,1);
			$this->routeData();
		}
		
		function wpmem_add_Websms_tab( $tabs ) {
			return array_merge( $tabs, array( 'Websms' => __( 'Websms', 'wp-members' ) ) );
		}
		
		function wpmem_Websms_panel()
		{
			echo '<div id="Websms-wpmem-panel" >
	...
	<fieldset>
		<legend>Please follow the below steps to enable OTP for WP Member Registration Form:</legend>
					...
					<ol >
						...
						<li>
							Create a new text field for Verification Code with meta key <strong>Websms_customer_validation_otp_token</strong>.
						</li>
					</ol>
		...
		function processPhoneAndStartOTPVerificationProcess($data)
		{
			...
			Websms_site_challenge_otp(null,'',null,$data['user_phone'],"phone",null,null,false);
		}
		...
		function validate_wpmember_submit($fields)
		{
			...
			do_action('Websms_validate_otp',NULL,$fields['Websms_customer_validation_otp_token']);
		}
		...
		function validate_submitted($fields)
		{
			...
			{	
				$wpmem_themsg =  WebsmsMessages::showMessage('INVALID_OTP');
				return false;
			}
			else
				return true;
		...
		function _add_shortcode_to_wpmember($mo_type,$field) 
		{
			...
			$field_content .= "font-family: Roboto;font-size: 12px !important;' id='Websms_otp_token_submit' ";
			...
			$field_content .= '<script>jQuery(document).ready(function(){$mo=jQuery;$mo("#Websms_otp_token_submit").click(function(o){ ';
			$field_content .= 'var e=$mo("input[name='.$field.']").val(); $mo("#mo_message").empty(),$mo("#mo_message").append("Sending OTP..."),';
			$field_content .= '$mo("#mo_message").show(),$mo.ajax({url:"'.site_url().'/?option=Websms-wpmember-form",type:"POST",';
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$wpmem_themsg =  WebsmsUtility::_get_invalid_otp_method();
		}
